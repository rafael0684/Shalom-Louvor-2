...
def apresentacaoPlaylist(request, playlist_id):
    playlist = get_object_or_404(Playlist, pk= playlist_id)
    musicas_da_playlist = playlist.playlist_musicas.all
    # musicas = musicas_da_playlist.filter(nome_artista = artista)
    # for musica in musicas_da_playlist:
    #   lyrics = musica.lyrics.split("£")


    contexto = {
        'playlist': playlist,
        'musicas_da_playlist': musicas_da_playlist,
